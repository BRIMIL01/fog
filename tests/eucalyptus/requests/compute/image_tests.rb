Shindo.tests('Fog::Compute[:eucalyptus] | image requests', ['eucalyptus']) do
  ...
  tests('success') do
    ...
    #   Fog::Compute[:eucalyptus].describe_images.body
    # end
    @image_id = 'ami-1aad5273'

    if Fog.mocking?
      @other_account = Fog::Compute::Eucalyptus.new(:eucalyptus_access_key_id => 'other', :eucalyptus_secret_access_key => 'account')

      tests("#register_image").formats(@register_image_format) do
        @image = Fog::Compute[:eucalyptus].register_image('image', 'image', '/dev/sda1').body
      end
      ...
      tests("#describe_images('Owner' => 'self')").formats(@describe_images_format) do
        Fog::Compute[:eucalyptus].describe_images('Owner' => 'self').body
      end

      tests("#describe_images('state' => 'available')").formats(@describe_images_format) do
        Fog::Compute[:eucalyptus].describe_images('state' => 'available').body
      end
      ...
      tests("#modify_image_attribute('#{@image_id}', 'Add.UserId' => ['#{@other_account.data[:owner_id]}'])").formats(@modify_image_attribute_format) do
        Fog::Compute[:eucalyptus].modify_image_attribute(@image_id, { 'Add.UserId' => [@other_account.data[:owner_id]] }).body
      end
      ...
      tests("#modify_image_attribute('#{@image_id}', 'Remove.UserId' => ['#{@other_account.data[:owner_id]}'])").formats(@modify_image_attribute_format) do
        Fog::Compute[:eucalyptus].modify_image_attribute(@image_id, { 'Remove.UserId' => [@other_account.data[:owner_id]] }).body
      end
    ...
    tests("#describe_images('image-id' => '#{@image_id}')").formats(@describe_images_format) do
      @other_image = Fog::Compute[:eucalyptus].describe_images('image-id' => @image_id).body
    end

    unless Fog.mocking?
      tests("#describe_images('Owner' => '#{@other_image['imageOwnerAlias']}', 'image-id' => '#{@image_id}')").formats(@describe_images_format) do
        Fog::Compute[:eucalyptus].describe_images('Owner' => @other_image['imageOwnerAlias'], 'image-id' => @image_id).body
      end
    end
  end
  ...
  tests('failure') do
    tests("#modify_image_attribute(nil, { 'Add.Group' => ['all'] })").raises(ArgumentError) do
      Fog::Compute[:eucalyptus].modify_image_attribute(nil, { 'Add.Group' => ['all'] }).body
    end

    tests("#modify_image_attribute('ami-00000000', { 'Add.UserId' => ['123456789012'] })").raises(Fog::Compute::Eucalyptus::NotFound) do
      pending unless Fog.mocking?

      Fog::Compute[:eucalyptus].modify_image_attribute('ami-00000000', { 'Add.UserId' => ['123456789012'] }).body
    end
  end
end

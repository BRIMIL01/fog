Shindo.tests('Fog::Compute[:eucalyptus] | instance requests', ['eucalyptus']) do
  ...
  tests('success') do
    ...
    key = Fog::Compute[:eucalyptus].key_pairs.create(:name => key_name)

    tests("#run_instances").formats(@run_instances_format) do
      data = Fog::Compute[:eucalyptus].run_instances(@ami, 1, 1, 'InstanceType' => 't1.micro', 'KeyName' => key_name).body
      @instance_id = data['instancesSet'].first['instanceId']
      data
    end

    server = Fog::Compute[:eucalyptus].servers.get(@instance_id)
    while server.nil? do
    ...
    tests("#describe_instances").formats(@describe_instances_format) do
       Fog::Compute[:eucalyptus].describe_instances('instance-state-name' => 'running').body
    end

    # Launch another instance to test filters
    another_server = Fog::Compute[:eucalyptus].servers.create

    tests("#describe_instances('instance-id' => '#{@instance_id}')").formats(@describe_instances_format) do
      body = Fog::Compute[:eucalyptus].describe_instances('instance-id' => @instance_id).body
    ...
    tests("#get_console_output('#{@instance_id}')").formats(@get_console_output_format) do
      Fog::Compute[:eucalyptus].get_console_output(@instance_id).body
    end

    tests("#get_password_data('#{@instance_id}')").formats(@get_password_data_format) do
      result = Fog::Compute[:eucalyptus].get_password_data(@instance_id).body
      ...
      tests("key can decrypt passwordData").returns(true) do
        ...
        Fog.wait_for do
          password_data ||= Fog::Compute[:eucalyptus].get_password_data(@instance_id).body['passwordData']
        end
    ...
    tests("#reboot_instances('#{@instance_id}')").formats(Eucalyptus::Compute::Formats::BASIC) do
      Fog::Compute[:eucalyptus].reboot_instances(@instance_id).body
    end

    tests("#terminate_instances('#{@instance_id}')").formats(@terminate_instances_format) do
      Fog::Compute[:eucalyptus].terminate_instances(@instance_id).body
    end

    tests("#describe_reserved_instances_offerings").formats(@describe_reserved_instances_offerings_format) do
      @reserved_instances = Fog::Compute[:eucalyptus].describe_reserved_instances_offerings.body
      @reserved_instances
    end

    tests('#describe_instance_status').formats(@describe_instance_status_format) do
      Fog::Compute[:eucalyptus].describe_instance_status.body
    end
    ...
    if Fog.mocking?
      ...
      tests("#purchase_reserved_instances_offering('#{@reserved_instance_offering_id}')").formats(@purchase_reserved_instances_offering_format) do
        Fog::Compute[:eucalyptus].purchase_reserved_instances_offering(@reserved_instance_offering_id, 1).body
      end

      tests("#describe_reserved_instances").formats(@describe_reserved_instances_format) do
        Fog::Compute[:eucalyptus].describe_reserved_instances.body
      end
    end
  end
  ...
  tests('failure') do
    ...
    tests("#get_console_output('i-00000000')").raises(Fog::Compute::Eucalyptus::NotFound) do
      Fog::Compute[:eucalyptus].get_console_output('i-00000000')
    end

    tests("#get_password_data('i-00000000')").raises(Fog::Compute::Eucalyptus::NotFound) do
      Fog::Compute[:eucalyptus].get_password_data('i-00000000')
    end

    tests("#reboot_instances('i-00000000')").raises(Fog::Compute::Eucalyptus::NotFound) do
      Fog::Compute[:eucalyptus].reboot_instances('i-00000000')
    end

    tests("#terminate_instances('i-00000000')").raises(Fog::Compute::Eucalyptus::NotFound) do
      Fog::Compute[:eucalyptus].terminate_instances('i-00000000')
    end

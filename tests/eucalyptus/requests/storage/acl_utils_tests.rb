require 'fog/eucalyptus/requests/storage/acl_utils'

Shindo.tests('Fog::Storage::Eucalyptus | ACL utils', ["eucalyptus"]) do
  tests(".hash_to_acl") do
    tests(".hash_to_acl({}) at xpath //AccessControlPolicy").returns("", "has an empty AccessControlPolicy") do
      xml = Fog::Storage::Eucalyptus.hash_to_acl({})
    ...
    tests(".hash_to_acl('Owner' => {}) at xpath //AccessControlPolicy/Owner").returns(nil, "does not have an Owner element") do
      xml = Fog::Storage::Eucalyptus.hash_to_acl('Owner' => {})
      Nokogiri::XML(xml).xpath("//AccessControlPolicy/Owner").first
    end
    ...
    tests(".hash_to_acl('Owner' => {'ID' => 'abcdef0123456789'}) at xpath //AccessControlPolicy/Owner/ID").returns("abcdef0123456789", "returns the Owner ID") do
      xml = Fog::Storage::Eucalyptus.hash_to_acl('Owner' => {'ID' => 'abcdef0123456789'})
    ...
    tests(".hash_to_acl('Owner' => {'DisplayName' => 'bob'}) at xpath //AccessControlPolicy/Owner/ID").returns(nil, "does not have an Owner ID element") do
      xml = Fog::Storage::Eucalyptus.hash_to_acl('Owner' => {'DisplayName' => 'bob'})
    ...
    tests(".hash_to_acl(#{acl.inspect}) at xpath //AccessControlPolicy/AccessControlList/Grant/Grantee").returns("CanonicalUser", "has an xsi:type of CanonicalUser") do
      xml = Fog::Storage::Eucalyptus.hash_to_acl(acl)
    ...
    acl = {
      'AccessControlList' => [
        {
          'Grantee' => {
            'URI' => 'http://acs.amazoneucalyptus.com/groups/global/AllUsers'
          },
          'Permission' => 'WRITE'
        }
      ]
    ...
    tests(".hash_to_acl(#{acl.inspect}) at xpath //AccessControlPolicy/AccessControlList/Grant/Grantee/URI").returns("http://acs.amazoneucalyptus.com/groups/global/AllUsers", "returns the Grantee URI") do
      xml = Fog::Storage::Eucalyptus.hash_to_acl(acl)
    ...
    tests(".hash_to_acl(#{acl.inspect}) at xpath //AccessControlPolicy/AccessControlList/Grant/Grantee/URI").returns("http://acs.amazoneucalyptus.com/groups/global/AllUsers", "returns the third Grant's Grantee URI") do
      xml = Fog::Storage::Eucalyptus.hash_to_acl(acl)
      Nokogiri::XML(xml).xpath("//AccessControlPolicy/AccessControlList/Grant/Grantee/URI").first.content
    end
  end
...
XML
    ...
    tests(".acl_to_hash(#{acl_xml.inspect})").returns({
      ...
      Fog::Storage::Eucalyptus.acl_to_hash(acl_xml)
    end
  end
end

require 'fog/core/collection'
require 'fog/eucalyptus/models/compute/key_pair'

module Fog
  module Compute
    class Eucalyptus

      class KeyPairs < Fog::Collection

        attribute :filters
        attribute :key_name

        model Fog::Compute::Eucalyptus::KeyPair

        # Used to create a key pair.  There are 3 arguments and only name is required.  You can generate a new key_pair as follows:
        # Eucalyptus.key_pairs.create(:name => "test", :fingerprint => "123", :private_key => '234234')
        #
        # ==== Returns
        #
        #<Fog::Eucalyptus::Compute::KeyPair
        #  name="test",
        #  fingerprint="3a:d3:e5:17:e5:e7:f7:de:fe:db:1b:c2:55:7d:94:0b:07:2e:05:aa",
        #  private_key="-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
        #>
        #
        # The key_pair can be retrieved by running Eucalyptus.key_pairs.get("test").  See get method below.
        #

        def initialize(attributes)
          self.filters ||= {}
          super
        end

        # Returns an array of all key pairs that have been created
        #
        # Eucalyptus.key_pairs.all
        #
        # ==== Returns
        #
        # <Fog::Eucalyptus::Compute::KeyPairs
        #  key_name=nil
        #  [
        #    <Fog::Eucalyptus::Compute::KeyPair
        #      name="test",
        #      fingerprint="1f:26:3d:83:e7:4f:48:74:c3:1c:e6:b3:c7:f6:ec:d8:cb:09:b3:7f",
        #      private_key=nil
        #    >
        #  ]
        #>
        #

        def all(filters = filters)
          unless filters.is_a?(Hash)
            Fog::Logger.deprecation("all with #{filters.class} param is deprecated, use all('key-name' => []) instead [light_black](#{caller.first})[/]")
            filters = {'key-name' => [*filters]}
          end
          self.filters = filters
          data = connection.describe_key_pairs(filters).body
          load(data['keySet'])
        end

        # Used to retrieve a key pair that was created with the Eucalyptus.key_pairs.create method.
        # The name is required to get the associated key_pair information.
        #
        # You can run the following command to get the details:
        # Eucalyptus.key_pairs.get("test")
        #
        # ==== Returns
        #
        #>> Eucalyptus.key_pairs.get("test")
        #  <Fog::Eucalyptus::Compute::KeyPair
        #    name="test",
        #    fingerprint="1f:26:3d:83:e7:4f:48:74:c3:1c:e6:b3:c7:f6:ec:d8:cb:09:b3:7f",
        #    private_key=nil
        #  >
        #

        def get(key_name)
          if key_name
            self.class.new(:connection => connection).all('key-name' => key_name).first
          end
        end

      end

    end
  end
end

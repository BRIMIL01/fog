Shindo.tests('Fog::Compute[:eucalyptus] | key pair requests', ['eucalyptus']) do

  tests('success') do

    @keypair_format = {
      'keyFingerprint'  => String,
      'keyName'         => String,
      'requestId'       => String
    }

    @keypairs_format = {
      'keySet' => [{
        'keyFingerprint' => String,
        'keyName' => String
      }],
      'requestId' => String
    }

    @key_pair_name = 'fog_create_key_pair'
    @public_key_material = 'ssh-rsa '

    tests("#create_key_pair('#{@key_pair_name}')").formats(@keypair_format.merge({'keyMaterial' => String})) do
      body = Fog::Compute[:eucalyptus].create_key_pair(@key_pair_name).body
      tests("key material").returns(OpenSSL::PKey::RSA, "is a valid private RSA key") do
        OpenSSL::PKey::RSA.new(body['keyMaterial']).class
      end
      body
    end

    tests('#describe_key_pairs').formats(@keypairs_format) do
      Fog::Compute[:eucalyptus].describe_key_pairs.body
    end

    tests("#describe_key_pairs('key-name' => '#{@key_pair_name}')").formats(@keypairs_format) do
      Fog::Compute[:eucalyptus].describe_key_pairs('key-name' => @key_pair_name).body
    end

    tests("#delete_key_pair('#{@key_pair_name}')").formats(Eucalyptus::Compute::Formats::BASIC) do
      Fog::Compute[:eucalyptus].delete_key_pair(@key_pair_name).body
    end

    tests("#import_key_pair('fog_import_key_pair', '#{@public_key_material}')").formats(@keypair_format) do
      Fog::Compute[:eucalyptus].import_key_pair('fog_import_key_pair', @public_key_material).body
    end

    tests("#delete_key_pair('fog_import_key_pair)").succeeds do
      Fog::Compute[:eucalyptus].delete_key_pair('fog_import_key_pair')
    end

    tests("#delete_key_pair('not_a_key_name')").succeeds do
      Fog::Compute[:eucalyptus].delete_key_pair('not_a_key_name')
    end

  end
  tests('failure') do

    @key_pair = Fog::Compute[:eucalyptus].key_pairs.create(:name => 'fog_key_pair')

    tests("duplicate #create_key_pair('#{@key_pair.name}')").raises(Fog::Compute::Eucalyptus::Error) do
      Fog::Compute[:eucalyptus].create_key_pair(@key_pair.name)
    end

    @key_pair.destroy

  end

end

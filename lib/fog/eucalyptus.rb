require 'fog/core'
require 'fog/eucalyptus/credential_fetcher'
require 'fog/eucalyptus/signaturev4'
module Fog
  module Eucalyptus
    ...
    def self.signed_params(params, options = {})
      params.merge!({
        'eucalyptusAccessKeyId'    => options[:eucalyptus_access_key_id],
      ...
      params.merge!({
        'SecurityToken'     => options[:eucalyptus_session_token]
      }) if options[:eucalyptus_session_token]
    ...
    class Mock
      ...
      def self.arn(vendor, account_id, path, region = nil)
        "arn:eucalyptus:#{vendor}:#{region}:#{account_id}:#{path}"
      end
